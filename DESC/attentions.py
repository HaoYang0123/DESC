...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, mask: Optional[Tensor] = None, stop_gradient_flag: Optional[bool] = False) -> Tuple[Tensor, Tensor]:
        if stop_gradient_flag:
            score = torch.bmm(query.detach(), key.detach().transpose(1, 2)) / self.sqrt_dim
        else:
        ...
        if stop_gradient_flag:
            context = torch.bmm(attn, value.detach())
        else:
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        context, attn = self.scaled_dot_attn(query, key, value, mask, stop_gradient_flag)
